...
import {TECHTALK, NOUPCOMING, URL404, RVA} from '../utils/constants';
...
export default class TechEvents extends Component {
  ...
  getAllTechTalks = () => {
    axios
      .get(settings.BACKEND_URL.concat('alltechtalks'))
      .then(res => {
        ...
        <HeaderImageScrollView
          ...
          renderFixedForeground={() => (
            <Animatable.View
              ...
              <Text style={styles.navTitle}>{TECHTALK}</Text>
            </Animatable.View>
          )}
          renderForeground={() => (
          ...
          <TriggeringView
            ...
            <Text style={styles.sectionTitle}>{TECHTALK}</Text>
          </TriggeringView>
          <NavigationEvents
            onWillFocus={() => {
          ...
          {rows?.length === 0 ? (
            ...
            rows?.map((l, i) => (
              <Card
                ...
                <Text style={styles.cardTitleEvent}>{l.topic}</Text>
                <Text style={styles.cardTitleEvent}>{l.presenter}</Text>
